...
from .feedback_generation import give_feedback, answer_followup_question, user_sessions
...
async def generate_feedback(
    ...
    if resource:
        try:
            ...
            resource_txt = resource_content.decode("utf-8", errors="ignore")
        except Exception:
            resource_txt = ""
    else:
        resource_txt = None
   
    feedback, session_id = give_feedback(user_id, subject, question, student_answer, resource_txt)
    print(feedback)
    return {"feedback": feedback, "session_id": session_id}

@app.post("/feedback/followup")
async def followup(
    payload: Dict[str, str]
):
    """
    Handle follow-up question for an existing session.
    Expects JSON: {"user_id": "...", "followup_question": "..."}
    """
    ...
    if not user_id or not followup_question:
        raise HTTPException(status_code=400, detail="Missing user_id or follow-up question.")

    # Retrieve the session ID for this user
    session_id = user_sessions.get(user_id)
    if not session_id:
        raise HTTPException(status_code=404, detail="No active feedback session for this user.")

    # Call your backend function to generate the follow-up answer
    response_text = answer_followup_question(session_id, followup_question)
